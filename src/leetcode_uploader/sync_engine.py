...
class SyncEngine:
    ...
    def _format_markdown(self, question_data: Dict, submission_data: Dict) -> str:
        ...
        # Pre-process: Strip <strong>/<b> tags inside <pre> blocks so they don't 
        # appear as literal '**' inside Markdown code blocks.
        def strip_bold_in_pre(match):
            content = match.group(0)
            content = re.sub(r'<(b|strong)>', '', content, flags=re.IGNORECASE)
            content = re.sub(r'<\/(b|strong)>', '', content, flags=re.IGNORECASE)
            return content
        
        html_content = re.sub(r'<pre>[\s\S]*?<\/pre>', strip_bold_in_pre, html_content)
...
| :--- | :--- | :--- | :--- | :--- |
| Accepted | {lang} | {runtime_info} | {memory_info} | {date_str} |
...
"""
    ...
    def sync(self, username: str, limit: int = 20):
        ...
        """
        print(f"🔍 Fetching your last {limit} accepted submissions...")
        ...
        print("\n✨ All done! Your GitHub repository is now synced.")
